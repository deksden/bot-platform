...
history:
  ...
  - version: 1.79.0
    date: 2026-04-21
    changes: Recorded waves 138-140: the twelfth target-repo doc packet is now landed, with SellerAgent operations specs (`deployment-architecture`, `production-rollout-runbook`, and `handoff-ownership-followup-lifecycle`, plus operations/spec hub linkage) migrated into `seller-agent`, framework engineering/security docs (`delivery-standards`, `coding-style`, and `auth-and-access` plus engineering/security/spec hub linkage) migrated into `bot-platform`, Docoved protocol docs (`PRT-033-docoved-semantic-container-fit-and-sources-readback` and `PRT-034-docoved-burst-continuity-finalization-and-verification-repair`, plus protocol/plans hub linkage) migrated into `docoved-agent`, and Wave 1B remaining-next wording narrowed to still-unlanded doc families.
  - version: 1.78.0
    date: 2026-04-21
    changes: Recorded waves 134-136: the eleventh target-repo doc packet is now landed, with SellerAgent runtime specs (`live-customer-burst-semantics` and `shadow-assist-and-message-review`, plus runtime/spec hub linkage) migrated into `seller-agent`, framework operations docs (`deployment-architecture`, `runbook`, `production-rollout-runbook`, and `hosted-beta-acceptance-contract` plus operations/spec hub linkage) migrated into `bot-platform`, Docoved protocol docs (`PRT-031-docoved-agentic-index-traversal-simplification-and-not-found-correction` and `PRT-032-docoved-agentic-ingest-wave-and-semantic-kb-rebuild`, plus protocol/plans hub linkage) migrated into `docoved-agent`, and Wave 1B remaining-next wording narrowed to still-unlanded doc families.
  - version: 1.77.0
    date: 2026-04-21
    changes: Recorded waves 130-132: the tenth target-repo doc packet is now landed, with SellerAgent commerce specs (`catalog-cart-deal-model` and `travelbook-business-playbook-and-seed`, plus commerce/spec hub linkage) migrated into `seller-agent`, framework protocol docs (`PRT-030-architecture-boundary-simplification-and-ownership-convergence` plus protocol hub linkage) migrated into `bot-platform`, Docoved protocol docs (`PRT-028-docoved-hierarchical-index-publication-and-tree-traversal-search` and `PRT-029-docoved-ordinary-root-index-and-directory-backed-knowledge-slices`, plus protocol/plans hub linkage) migrated into `docoved-agent`, and Wave 1B remaining-next wording narrowed to still-unlanded doc families.
  - version: 1.76.0
    date: 2026-04-21
...
- framework ADR packet is landed:
  ...
  - `plans/adr/index.md`
- remaining backlog is the still-unlanded framework project/spec/scenario/protocol/ADR families beyond the twelve landed packets.
...
- protocol/plans packet is landed:
  ...
  - `plans/protocols/index.md`
  - `plans/index.md`
- remaining backlog is the broader Docoved architecture/runtime/operations/plans/guides/reference families beyond the twelve landed packets.

Must cover:
- Docoved architecture/runtime/operations docs
- `EP-023`, `ADR-005..006`, and remaining `PRT-033..035`
...
- the active protocol focus is now continued Wave `1B` repo-local doc migration plus later bounded code-move packets, with thirteen target-repo documentation packets already landed across `seller-agent`, `bot-platform`, and `docoved-agent`;
...
- mixed `sales-agent/.memory-bank/**` hubs now route readers toward target-repo canonical truth where needed;
- the first canonical source-doc families are migrated into their target repos;
- placeholder-level feature/scenario structure has been replaced with repo-local actualized content in all three target repos.
...
- the first real Wave 2 extraction slice is now landed:
...
- the extracted framework-core package is now also materialized in release-ready versioned repo state:
...
- remaining follow-up is no longer an owner-side Wave 6 blocker, but wave 172 confirmed it is deeper than dead import-path cleanup alone: later work still needs to extract the shared runnable-local scenario framework contour, place shared auth/observed-user store seams cleanly because some `docoved-agent` operator scripts still assume missing repo-local `createAuthStore` / `createConversationStore` exports, localize the remaining Docoved corpus/Telegram script helpers without reviving mixed `apps/server` coupling, and retire residual mixed bootstrap storage habits.
